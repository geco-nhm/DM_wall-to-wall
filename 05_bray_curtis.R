...
##
## Notes:
## this script is the fifth and the last stage in the process of producing a WALL-to-wall map of Vegetation types
## here we evaluate the three assembled maps using BRAY CURTIS - proportional dissimilarity index
## 
##
## ------------------
#### 00 ####
...
#### 6.oct 2020 ####
#### Bray-curtis for VT and EG - 1081plots####


#reusing the funciton Raster to table
## function RASTER to VT/EG table ####
rast_to_table_1.5 <- function(rast, plots){
  rast_mask_df <- extract(rast, plots, df=TRUE)
  # make table
  print("finished extracting")
  rast_mask_table <- as.data.frame.matrix(t(table(rast_mask_df, useNA = "always"))) # transpose columns and rows and save summarized table, 
  colnames(rast_mask_table) <- paste0("X", plots@data$FLATE_NR ) # assign column names with plot ID
  print("saving AR rectangles")
  # Save output
  write.csv2(rast_mask_table, paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/",names(rast),".csv"))
  saveRDS(rast_mask_table, paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/",names(rast)))
  #writeRaster(rast_mask,filename=paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/",names(rast)), format="GTiff", overwrite=TRUE)
  #return(rast_mask_table)
}
####EG####
#load rasters for use in function
#loading in the three layers DM_maxval, DM_preval and DM_AUC
AR18_DM_Maxval_GRP <- raster("E:/Project_1.5/OUTPUT_agri/EG/EG_AR18_DM_maxval.tif")
AR18_DM_Preval_GRP <- raster("E:/Project_1.5/OUTPUT_agri/EG/EG_AR18_DM_preval.tif")
AR18_DM_AUC_GRP <- raster("E:/Project_1.5/OUTPUT_agri/EG/EG_AR18_DM_AUC.tif")
#loading in reference AR18x18
AR18_ref_GRP <- raster("E:/Project_1.5/OUTPUT_agri/EG/EG_AR18_ref.tif")
veg_plots <- readOGR(in_veg_data, "Hui_dissolved_plots")
# executing rast_to_table#
rast_to_table_1.5(AR18_DM_Maxval_GRP,veg_plots)
rast_to_table_1.5(AR18_DM_AUC_GRP,veg_plots)
rast_to_table_1.5(AR18_DM_Preval_GRP,veg_plots)
rast_to_table_1.5(AR18_ref_GRP,veg_plots)
# combining to a list
GRP_list <-list(
  AR18_DM_Maxval_GRP_df <- readRDS(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_AR18_DM_maxval")),
  AR18_DM_AUC_GRP_df <- readRDS(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_AR18_DM_AUC")),
  AR18_DM_Preval_GRP_df <- readRDS(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_AR18_DM_preval")),
  AR18_DM_ref_df <- readRDS(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_AR18_ref"))
    ) #comp_DM_maxval, comp_DM_preval,
names(GRP_list) <- c("DM_maxval","DM_AUC","DM_preval","AR18x18")

###clean "NA" columns, these are where all area is Excluded and its' not possible to calculate percentage from 0
library(reshape2)
library(data.table)
GRP_list_df <- rbindlist(GRP_list, idcol = TRUE) # this reforms the list into dataframe, taking first/identical columns from each list
saveRDS(GRP_list_df,"E:/Project_1.5/OUTPUT_agri/bray_curtis/GRP_list_df")
#when both packages reshape and data.table are attached, generic base functions sometimes fail - detach if not in use
#detach("package:reshape2", unload=TRUE)
#detach("package:data.table", unload=TRUE)
#GRP_list_df<- readRDS("E:/Project_1.5/OUTPUT_agri/bray_curtis/GRP_list_df")
t<-GRP_list_df
# t consists of 4 lists bound together, row 1:12 is EGs and 13is NAs
# eventually we could delete each 13th row, to keep only valid data.... WORTH EXPLORING!!!
t <- t[-c(13,26,39,52),1:15]
t<-t[, colSums(is.na(t)) == 0]
# only 1025 out of 1081 columns are left
# 56 colums were excluded because of NA values (*in other words sites where one of the methods was totally EXCLUDED)

    # t[ , apply(t, 2, function(x) !any(is.na(x)))]
    # t[,!apply(is.na(t), 2, any)]
    # t[,complete.cases(t(t))] 
    # t<-lapply(PFT_method_1081, "[", 4:10)
    # testik <- lapply(t, function(df) df[, colSums(is.na(df)) == 0])

# now we need to reassign DF into list
t[1:13,1:5] #-c(1:2)
t[14:26,1:5]
t[27:39,1:5]
t[40:52,1:5]
...
GRP_list_1081 <-list(t[1:13,-c(1)], t[14:26,-c(1)], t[27:39,-c(1)], t[40:52,-c(1)]) #comp_DM_maxval, comp_DM_preval,
names(GRP_list_1081) <- c("DM_maxval","DM_AUC", "DM_preval","AR18x18")
# GRP_list_1081<-lapply(GRP_list_1081, function(x){
#   rownames(x) <- t(comp_DM_AUC[,2])
#   return(x)
# })
# lapply(PFT_method_1081, "[", 4)

# #### rowsum 1081 ####
# norway_PFT_1081 <- as.data.frame(lapply(PFT_method_1081, rowSums))
# #norway_PFT_1081 <- format(norway_PFT_1081, digits=1)
# #colSums(norway_PFT_1081)
# saveRDS(norway_PFT_1081, file = paste0(path, "/Norway_PFT_1081.RDS"))
# #Norway_PFT_1081 <- readRDS("E:/Project_2_RUNS/OUTPUT/results/Norway_PFT_1081.RDS")
# write.csv2(norway_PFT_1081, file = paste0(path, "/Norway_PFT_1081.csv"))
# #### vegdist on the whole dataset
# vegdist(t(norway_PFT))
# vegdist(t(norway_PFT_1081))
# run vegdist for each site
bray_curtis1081 <- function(site){
  site_1081<- t(as.data.frame(lapply(GRP_list, "[", site)))
  rownames(site_1081) <- c("DM_maxval","DM_AUC","DM_preval", "AR18x18")
  #colnames(site_1081) <- t(comp_DM_AUC[,2])
  bray_site_1081<-vegdist(site_1081, method = "bray")
  return(bray_site_1081)
}
...
# since only 1025 out of 1081 columns are left
# 56 colums were excluded because of NA values 
#(*in other words sites where one of the methods was totally EXCLUDED)

a <- vector('list', length(2))
for(i in 1:1082){
  a[[i]] <- bray_curtis1081(i)
  names(a)[i] <- colnames(GRP_list[[1]])[i]
}
# preparing the output 
a[[3]]
a[[3]][c(3,5,6)] # we need the bottom row where each method is compared to AR18x18
mypos <- c(3,5,6) #positions for BC dissimilarity of AR18x18 to each of the other methods
...
#rowMeans(bc_dis)
colMeans(bc_dis, na.rm = TRUE)
bc_dis <- cbind(rownames(bc_dis),bc_dis)
colnames(bc_dis) <- c("plot_nr", "DM_maxval", "DM_AUC", "DM_preval")
write.csv2(bc_dis, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_braycurtis_1081.csv"))
saveRDS(bc_dis, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_braycurtis_1081.RDS"))
#EG_bc_dis <- readRDS("E:/Project_1.5/OUTPUT/bray_curtis/EG_braycurtis_1081.RDS")
#summary(EG_bc_dis[,2:4])
boxplot(bc_dis[2:4])
library(reshape2)
bc_dis_l <- melt(bc_dis)
colnames(bc_dis_l) <- c("plot_nr","Method", "Bray_Curtis")
saveRDS(bc_dis_l, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_braycurtis_long_1081.RDS"))
write.csv2(bc_dis_l, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/EG_braycurtis_long_1081.csv"))
#bc_dis_l <- readRDS("E:/Project_1.5/OUTPUT/bray_curtis_agri/EG_braycurtis_long_1081.RDS")

#### ploting ####
library(ggplot2)
#set path to save output plots
path = "C:/Users/peterhor/OneDrive - Universitetet i Oslo/Paper 1.5/"
## all methods including "DM_maxval", "DM_preval"
bc_plot1 <- ggplot(bc_dis_l, aes(x=Method, y=Bray_Curtis, fill=Method)) + 
  geom_boxplot(alpha=0.8) +
  geom_point(aes(fill = Method), alpha=0.05, size = 3, shape = 21, position = position_jitterdodge()) +
  labs(title="Ecosystem group level", 
       x="Modelling method",
       y="Proportional dissimilarity") 
bc_plot1
ggsave(filename = "Project_1.5_bc_EG_1081.png",plot = bc_plot1, device = "png", path = paste0(path, "/ggplots"),dpi = 300 )

# VIOLIN Plots
bc_plot2 <- ggplot(bc_dis_l, aes(x=Method, y=Bray_Curtis, fill=Method)) + 
  ...
  labs(title="Ecosystem group level",
       x="Modelling method",
       y="Proportional dissimilarity") 
  #+geom_line(aes(group=plot_nr), colour="grey",  alpha=0.1)
  
bc_plot2
ggsave(filename = "Project_1.5_bc_EG_violin_1081.png",plot = bc_plot2, device = "png", path = paste0(path, "/ggplots"),dpi = 300 )
...
#load rasters for use in function
#loading in the three layers DM_maxval, DM_preval and DM_AUC
AR18_DM_Maxval_VT <- raster("E:/Project_1.5/OUTPUT_agri/VT/VT_AR18_DM_maxval.tif")
AR18_DM_AUC_VT <- raster("E:/Project_1.5/OUTPUT_agri/VT/VT_AR18_DM_AUC.tif")
AR18_DM_Preval_VT <- raster("E:/Project_1.5/OUTPUT_agri/VT/VT_AR18_DM_preval.tif")

#loading in reference AR18x18
AR18_ref_VT <- raster("E:/Project_1.5/OUTPUT_agri/VT/VT_AR18_ref.tif")
veg_plots <- readOGR(in_veg_data, "Hui_dissolved_plots")
# executing rast_to_table#
rast_to_table_1.5(AR18_DM_Maxval_VT,veg_plots)
rast_to_table_1.5(AR18_DM_AUC_VT,veg_plots)
rast_to_table_1.5(AR18_DM_Preval_VT,veg_plots)
rast_to_table_1.5(AR18_ref_VT,veg_plots)
# combining to a list

# empty rows for VT 18, etc had to be filled
AR18_DM_maxval_VT_df <- read.csv(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_AR18_DM_maxval.csv"))
AR18_DM_AUC_VT_df <- read.csv(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_AR18_DM_AUC.csv"))
AR18_DM_preval_VT_df <- read.csv(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_AR18_DM_preval.csv"))
AR18_DM_ref_VT_df <- read.csv2(paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_AR18_ref.csv"))
# some categories were not represented. In ordre to have same size of dataframe, i.e. 32rows we had to insert zeros for the missing classes. (done in Excel) 
# read.csv. for all edited, read.csv2 for orig # this is due to system settings with comma separator.


# read in list
VT_list <-list(AR18_DM_maxval_VT_df, AR18_DM_AUC_VT_df, AR18_DM_preval_VT_df, AR18_DM_ref_VT_df) #comp_DM_maxval, comp_DM_preval,
names(VT_list) <- c("DM_maxval","DM_AUC","DM_preval","AR18x18")

###clean "NA" columns, these are where all area is Excluded and its' not possible to calculate percentage from 0
library(reshape2)
library(data.table)
VT_list_df <- rbindlist(VT_list, idcol = TRUE)
saveRDS(VT_list_df,"E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_list_df")
#detach("package:reshape2", unload=TRUE)
#detach("package:data.table", unload=TRUE)
# #VT_list_df<- readRDS("E:/Project_1.5/OUTPUT/bray_curtis/VT_list_df")
# t<-VT_list_df
# t<-t[, colSums(is.na(t)) == 0]
# # only 1025 out of 1081 columns are left
# # 56 colums were excluded because of NA values (*in other words sites where one of the methods was totally EXCLUDED)
...
# 
# 
# VT_list_1081 <-list(t[1:32,-c(1:2)], t[33:64,-c(1:2)], t[65:96,-c(1:2)], t[97:128,-c(1:2)]) #comp_DM_maxval, comp_DM_preval,
# names(VT_list_1081) <- c("DM_maxval", "DM_preval","DM_AUC", "AR18x18")
# VT_list_1081<-lapply(VT_list_1081, function(x){
#   rownames(x) <- t(comp_DM_AUC[,2])
#   return(x)
# })
# lapply(PFT_method_1081, "[", 4)
...
# run vegdist for each site
bray_curtis1081 <- function(site){
  site_1081<- t(as.data.frame(lapply(VT_list, "[", site)))
  rownames(site_1081) <- c("DM_maxval", "DM_AUC", "DM_preval", "AR18x18")
  #colnames(site_1081) <- t(comp_DM_AUC[,2])
  bray_site_1081<-vegdist(site_1081, method = "bray")
  return(bray_site_1081)
...
for(i in 2:1082){ 
  a[[i]] <- bray_curtis1081(i)
  names(a)[i] <- colnames(VT_list[[1]])[i]
}
# preparing the output 
#a[[1]] <- NULL # removing first element from list
a[[3]][c(3,5,6)] # notice different position than in 20 sites
mypos <- c(3,5,6) #positions for BC dissimilarity of AR18x18 to each of the other methods
#extracting positions from each list
# sapply(a, "[", mypos)
# lapply(a, "[", mypos)
...
bc_dis <- cbind(rownames(bc_dis),bc_dis)
colnames(bc_dis) <- c("plot_nr", "DM_maxval","DM_AUC", "DM_preval")
write.csv2(bc_dis, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_braycurtis_1081.csv"))
saveRDS(bc_dis, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_braycurtis_1081.RDS"))
#boxplot(bc_dis[2:4])
#VT_bc_dis <- readRDS("E:/Project_1.5/OUTPUT/bray_curtis/VT_braycurtis_1081.RDS")
#summary(VT_bc_dis[,2:4])
library(reshape2)
bc_dis_l <- melt(bc_dis)
colnames(bc_dis_l) <- c("plot_nr","Method", "Bray_Curtis")
saveRDS(bc_dis_l, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_braycurtis_long_1081.RDS"))
write.csv2(bc_dis_l, file = paste0("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_braycurtis_long_1081.csv"))
bc_dis_l <- readRDS("E:/Project_1.5/OUTPUT_agri/bray_curtis/VT_braycurtis_long_1081.RDS")

#### ploting ####
library(ggplot2)
path = "C:/Users/peterhor/OneDrive - Universitetet i Oslo/Paper 1.5/"
## all methods including "DM_maxval", "DM_preval"
bc_plot3 <- ggplot(bc_dis_l, aes(x=Method, y=Bray_Curtis, fill=Method)) + 
  geom_boxplot(alpha=0.8) +
  geom_point(aes(fill = Method), alpha=0.05, size = 3, shape = 21, position = position_jitterdodge()) +
  labs(title="Vegetation type level",
       x="Modelling method",
       y="Proportional dissimilarity") 
bc_plot3
ggsave(filename = "Project_1.5_bc_VT_1081.png",plot = bc_plot3, device = "png", path = paste0(path, "/ggplots"),dpi = 300 )

# with violin
bc_plot3.1 <- ggplot(bc_dis_l, aes(x=Method, y=Bray_Curtis, fill=Method)) + 
  geom_violin(alpha=0.8) +
  geom_boxplot(width=0.1) +
  geom_point(aes(fill = Method), alpha=0.05, size = 3, shape = 21, position = position_jitterdodge()) +
  labs(title="Vegetation type level",
  ...
  #+geom_line(aes(group=plot_nr), colour="grey",  alpha=0.1)
bc_plot3.1
ggsave(filename = "Project_1.5_bc_VT_violin_1081.png",plot = bc_plot3.1, device = "png", path = paste0(path, "/ggplots"),dpi = 300 )
